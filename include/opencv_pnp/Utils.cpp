...
void createFeatures(const std::string &featureName, int numKeypoints, cv::Ptr<cv::Feature2D> &detector, cv::Ptr<cv::Feature2D> &descriptor)
{
    ...
    {
        // SIFT is not available in standard OpenCV builds
        // It requires opencv_contrib (xfeatures2d) module
        // For now, default to ORB which is always available
        std::cout << "SIFT is not available in this OpenCV build (requires opencv_contrib)." << std::endl;
